...
{
	const int USER_HEAP_SIZE = USER_HEAP_MAX - USER_HEAP_START;
	...
	MAX_MEM_BLOCK_CNT = USER_HEAP_SIZE/PAGE_SIZE;

	MemBlockNodes= (struct MemBlock*) USER_DYN_BLKS_ARRAY;

	uint32 MemSize = ROUNDUP((MAX_MEM_BLOCK_CNT * sizeof(struct MemBlock)), PAGE_SIZE);
	sys_allocate_chunk(USER_DYN_BLKS_ARRAY,MemSize,PERM_USER|PERM_WRITEABLE);

	//	  (remember to set MAX_MEM_BLOCK_CNT with the chosen size of the array)
	//[3] Initialize AvailableMemBlocksList by filling it with the MemBlockNodes
	initialize_MemBlocksList(MAX_MEM_BLOCK_CNT);

	struct MemBlock *UHEAP = LIST_FIRST(&AvailableMemBlocksList);
	LIST_REMOVE(&AvailableMemBlocksList, UHEAP);
	UHEAP->size = USER_HEAP_SIZE;
	UHEAP->sva = USER_HEAP_START;
	//[4] Insert a new MemBlock with the heap size into the FreeMemBlocksList
	LIST_INSERT_HEAD(&FreeMemBlocksList,UHEAP);
}
...
void* malloc(uint32 size)
{
	...
	if(allocated_block !=NULL)
	{
		...
		sys_allocate_user_mem(allocated_block->sva,size);
...
//	panic("free() is not implemented yet...!!");
	...
	sys_free_user_mem((unsigned int)va, block->size);*/
...
//	panic("smalloc() is not implemented yet...!!");
	...
	size=ROUNDUP(size,PAGE_SIZE);
		...
		if(allocated_block !=NULL)
		{
			int index = sys_createSharedObject(sharedVarName,size,isWritable,(void*)allocated_block->sva);
			if (index >= 0) {
				insert_sorted_allocList(allocated_block);
				return (void*)allocated_block->sva;
			}
...
//	panic("sget() is not implemented yet...!!");
	...
	if (sharedSize == E_SHARED_MEM_NOT_EXISTS) return NULL;
	struct MemBlock *allocated_block;
	if(sys_isUHeapPlacementStrategyFIRSTFIT())
		allocated_block = alloc_block_FF(sharedSize);
	if(allocated_block == NULL)return NULL;
	int index = sys_getSharedObject(ownerEnvID,sharedVarName,(void*)allocated_block->sva);
	if (index >= 0) return (void*)allocated_block->sva;
	return NULL;
}
...
void sfree(void* virtual_address)
{
	...
	struct MemBlock * block = find_block(&AllocMemBlocksList,va);
	if(block != NULL){
		int freeRet = sys_freeSharedObject(,va);
//		sys_free_user_mem(va , block->size);
		if(freeRet == 0){
			LIST_REMOVE(&AllocMemBlocksList,block);
			insert_sorted_with_merge_freeList(block);
