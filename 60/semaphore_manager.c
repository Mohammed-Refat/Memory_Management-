...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	int semExist = get_semaphore_object_ID(ownerEnvID,semaphoreName);
	if(semExist == E_SEMAPHORE_NOT_EXISTS){
		struct Semaphore * temp_Semaphore = NULL;
		int semIndex = allocate_semaphore_object(&temp_Semaphore);
		if(semIndex == E_NO_SEMAPHORE)
			//	c) E_NO_SEMAPHORE if the the array of semaphores is full
			return E_NO_SEMAPHORE;

		semaphores[semIndex].value = initialValue;
		semaphores[semIndex].ownerID = ownerEnvID;
		strcpy(semaphores[semIndex].name,semaphoreName);
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	int semIndex = get_semaphore_object_ID(ownerEnvID,semaphoreName);
	semaphores[semIndex].value--;
	if(semaphores[semIndex].value < 0){
		enqueue(&semaphores[semIndex].env_queue,myenv);
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	semaphores[semIndex].value++;
	if(semaphores[semIndex].value <= 0){
		newEnv = dequeue(&semaphores[semIndex].env_queue);
		sched_insert_ready(newEnv);
		newEnv->env_status = ENV_READY;
	}
}
