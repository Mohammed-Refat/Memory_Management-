...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	int shareExist = get_share_object_ID(ownerID,shareName);
	if(shareExist == E_SHARED_MEM_NOT_EXISTS){
		struct Share* temp_shared;
		int shareIndex = allocate_share_object(&temp_shared);

		//	c) E_NO_SHARE if the number of shared objects reaches max "MAX_SHARES"
		if(shareIndex == E_NO_SHARE)
			return E_NO_SHARE;
		...
		while(sva < eva){
			...
			frame->va = sva;
			map_frame(myenv->env_page_directory,frame,sva,PERM_WRITEABLE|PERM_USER|PERM_PRESENT);
			add_frame_to_storage(shares[shareIndex].framesStorage,frame,i);
			i++;
			sva+=PAGE_SIZE;
		}
		shares[shareIndex].ownerID = ownerID;
		strcpy(shares[shareIndex].name,shareName);
		shares[shareIndex].size = size;
		shares[shareIndex].isWritable = isWritable;
		shares[shareIndex].references = 1;
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	int sharedIndex = get_share_object_ID(ownerID,shareName);

	//	b) E_SHARED_MEM_NOT_EXISTS if the shared object is not exists
	if (sharedIndex == E_SHARED_MEM_NOT_EXISTS)
			return E_SHARED_MEM_NOT_EXISTS;
	uint32 sva = ROUNDDOWN((uint32)virtual_address,PAGE_SIZE);
	uint32 eva = ROUNDUP((uint32)virtual_address+shares[sharedIndex].size,PAGE_SIZE);
	int i = 0;
	int perms;
	if(shares[sharedIndex].isWritable) perms = PERM_WRITEABLE|PERM_USER|PERM_PRESENT;
	else perms = PERM_USER|PERM_PRESENT;
	while(sva < eva){
		struct FrameInfo* frame = get_frame_from_storage(shares[sharedIndex].framesStorage,i);
		map_frame(myenv->env_page_directory,frame,sva,perms);
		i++;
		sva+=PAGE_SIZE;
	}
	shares[sharedIndex].references ++;
